...
impl Order {
    ...
    fn dish_count(&self, dish: Dish) -> u32 {
        self.dishes.iter().filter(|&d| *d == dish).count() as u32
    }
    ...
    fn total(&self) -> u32 {
        let sum = self.dish_count(Dish::ThaiChicken) * Dish::ThaiChicken.price()
            + self.dish_count(Dish::Tofu) * Dish::Tofu.price()
            + self.dish_count(Dish::FriedRice) * Dish::FriedRice.price();
